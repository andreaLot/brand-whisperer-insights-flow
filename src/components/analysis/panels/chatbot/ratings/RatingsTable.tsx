...
const RatingsTable: React.FC<RatingsTableProps> = ({ 
  ...
  useEffect(() => {
    ...
    if (platformResults.length > 0) {
      ...
      setTimeout(() => setLegendVisible(true), 700 + (results.length * 200) + 300);
      setTimeout(() => setFooterVisible(true), 700 + (results.length * 200) + 800);
    } 
    else if (showSkeleton) {
      setIsLoading(true);
    }
  }, [platformResults.length, results.length, showSkeleton]);
