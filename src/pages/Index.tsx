...
const Index = () => {
  ...
  return <div className="min-h-screen flex flex-col items-center justify-center bg-brand-black p-4 text-white">
      <div className="w-full max-w-5xl flex flex-col md:flex-row gap-8">
        ...
        <div className="w-full md:w-1/2 flex flex-col gap-6">
          <ConversationBubble>
            ...
            {step === 'business-name' && <div className="space-y-10">
                <h2 className="text-xl font-normal">
            ...
            {step === 'location' && <div className="space-y-10">
                <h2 className="text-xl font-normal">
            ...
            {step === 'category-detection' && <div className="space-y-6">
                <h2 className="text-xl font-normal">
            ...
            {step === 'analyzing' && <div className="space-y-6">
                <h2 className="text-xl font-normal">
            ...
            {step === 'results' && analysisResult && <div className="space-y-6">
                <h2 className="text-xl font-normal">
                ...
                <Button onClick={handleStartOver} variant="dynamic" size="xl" className="mt-4">
                  Start New Analysis
                </Button>
              </div>}
          </ConversationBubble>
        ...
        <div className="w-full md:w-1/2">
          {(step === 'welcome' || step === 'business-name') && <SearchInput placeholder="Search for your business" value={businessName} onChange={setBusinessName} onSubmit={handleBusinessNameSubmit} />}
          
          {step === 'location' && <LocationSelector onSelect={handleLocationSelect} />}
          
          {step === 'category-detection' && suggestedCategories.length > 0 && <div className="bg-brand-gray-dark rounded-lg p-6 border border-gray-700 animate-fade-in">
              <h3 className="text-lg font-medium mb-4">Detected Categories:</h3>
              <ul className="space-y-2">
                {suggestedCategories.map((cat, index) => <li key={index} className="flex items-center justify-between">
                    <span>{cat.name}</span>
          ...
          {step === 'results' && analysisResult && <div className="space-y-6">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {analysisResult.platformResults.map((result, index) => <ResultCard key={index} platform={result.platform} score={result.score} rank={result.rank} icon={renderPlatformIcon(result.platform)} />)}
              </div>
              
              <SummaryBox overallScore={analysisResult.overallScore} strengths={analysisResult.strengths} weaknesses={analysisResult.weaknesses} recommendations={analysisResult.recommendations} />
            </div>}
        </div>
      </div>
